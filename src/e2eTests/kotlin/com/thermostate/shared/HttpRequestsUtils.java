...
public class HttpRequestsUtils {
    ...
    public static void createSingleUser(E2EDB db) {
        ...
        db.given("""
                insert into users (name, password, salt, email, active, uuid, type) values (
                'Amaia',
    ...
    public static void createUser(String name, String password, String email, String userType) {
        E2ERequest
                .to("http://127.0.0.1:8080/user")
                //.withABearer(HttpRequestsUtils::getBearer)
                .withHeader("Authorization", getBearer("Amaia", "pass"))
                .withContentType("application/json;charset=UTF-8")
                .sendAPost(Map.of("name", name, "password", password, "email", email, "userType", userType))
